...
async function aiRoutes(fastify) {
    ...
    fastify.post('/chat', {
        ...
        try {
            ...
            const allDirectory = await prisma_js_1.default.directoryEntry.findMany({ take: 1000 });
            ...
            if (relevantContacts.length === 0 || (relevantContacts.length > 1 && relevantContacts[0].searchScore < 500)) {
                const fuzzyMatches = allDirectory
                    .map((c) => ({
                    name: c.name,
                    score: Math.max(...searchWords.map((w) => 1 - AI_UTILS.levenshtein(c.name.toLowerCase().split(' ')[0], w) / Math.max(c.name.split(' ')[0].length, w.length)))
                }))
